...
def rank_df(computed_df: pd.DataFrame, computed_col: str):
    score_col = "score"
    score_df = computed_df.sort_values(computed_col, ascending=True)

    bin_len = len(score_df) // 4
    if len(score_df) % 4 != 0:
        bin_len += 1

    score_df[score_col] = [i // bin_len + 1 for i in range(len(score_df))]
    score_df = score_df.sort_values(score_col, ascending=True)
    return score_df, score_col


def rank_df_follow_minmax(computed_df: pd.DataFrame, computed_col: str):
...
if __name__ == "__main__":
    ...
    output_path = 'output/scored_provinces.csv'
    score_df.drop(columns=['STT']).to_csv(output_path, index=False)
    print(f"[+] Saved scored provinces to: {output_path}")
